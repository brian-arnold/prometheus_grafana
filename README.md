...
Setting up for DirectPV was pretty straightforward as the scrape config they supply can be copy/pasted into the values file under additionalScrapeConfigs.
...
2. Put scrape config in the values file
  ...
  - Instead I put the following in a file named `minio-scrape-config-w-token.yaml`, which is the output from the command above
  ```
  - job_name: minio-job
    bearer_token: <TOKEN> 
  ...
  ```
  - Save this in a file called `minio-scrape-config-w-token.yaml` to store in a secret
  ```
  kubectl create secret generic additional-scrape-configs-minio \
  --from-file=prometheus-minio.yaml=minio-scrape-config-w-token.yaml \
  -n monitoring
  ```
